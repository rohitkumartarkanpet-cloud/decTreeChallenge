...
This report explores how decision trees measure feature importance, and why the way we encode categorical variables—such as **zip codes**—can significantly shape our interpretation of a model.  
...
- **zipCode** — a *non-ordinal categorical variable*
- Square footage  
- Lot area  
- Year built  
- Bedrooms  
...
# 🌲 Model 1 — zipCode Treated as **Numerical**
...
## ⭐ Feature Importance (Numerical Model)
...
plt.title("Feature Importance — Numerical zipCode")
...
# 🌲 Model 2 — zipCode One-Hot Encoded (Proper Categorical)
...
## 📸 Categorical Tree Visualization
...
## ⭐ Feature Importance (Categorical Model)
...
plt.title("Feature Importance — Categorical zipCode")
plt.gca().invert_yaxis()
plt.show()
```
...
## **1️⃣ Numerical vs Categorical Encoding — How Should zipCode Be Modeled?**

Zip codes represent **distinct neighborhoods**.  
They **do not** have a meaningful numerical sequence—**50010 is not “greater”** than 50011 in any way relevant to housing prices.
...
- meaningless splits like “zipCode < 50012.5”  
...
## **2️⃣ R vs Python — Why Are the Results Different?**

### ✔ Key Difference  
R’s `rpart` natively supports **true categorical splits** using *multi-way partitions*:

> *“Categorical predictors are handled using multi-way splits.”* — rpart documentation  

In our experiment, the overall fit of the numeric and categorical trees is similar; the main difference is how clearly ZIP’s neighborhood effect can be read from the tree.

### ⚠️ Scikit-learn Limitation (Official Documentation Quote)

From scikit-learn:

> **“The DecisionTreeClassifier and DecisionTreeRegressor do not support categorical features.”**
...
### ✔ Which language handles categorical variables better?

**R** — because it performs natural multi-way categorical splitting without encoding workarounds.

---

## **3️⃣ Better Categorical Tree Options in Python**
...
### ⭐ **2. CatBoost**  
> “CatBoost handles categorical features internally using efficient target statistics.”  
Documentation: <https://catboost.ai/en/docs/>

### ⭐ **3. PyTorch Tabular**  
> “Supports categorical embeddings without manual encoding.”  
Documentation: <https://pytorch-tabular.readthedocs.io/>

These approaches avoid manually exploding the feature space while still modeling categorical structure.
...
- Modern Python libraries (LightGBM, CatBoost, PyTorch Tabular) handle categories far better than plain scikit-learn trees  
- Decision tree interpretations change drastically depending on encoding, even when overall accuracy looks similar  

For a business stakeholder, the key takeaway is simple: **if ZIP is encoded correctly, the model clearly shows which neighborhoods matter for price; if it is encoded as a number, that signal is hidden or distorted.**
...
```{=html}
<style>
/* Make the "Parameters" line in the small card readable */
details > summary {
