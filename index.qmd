...
1) **Diverging delta bars**: (Categorical minus Numeric) importance share per feature, sorted by absolute change.  
2) **Compact delta table**: shows Numeric vs Categorical with an up/down arrow and a colored delta.
...
- Python (`sklearn.tree.DecisionTreeRegressor`) requires numeric input. Categorical features must be encoded (one‑hot or ordinal). This encoding changes the split search space and how importance credit is distributed across dummies.
...
When categories carry signal (like ZIP), prefer models with native categorical support:
