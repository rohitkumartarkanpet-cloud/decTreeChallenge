...
# Order features by average rank for a tidy layout (largest average rank at top for readability)
ranks['avg'] = (ranks['Rank_Numeric'] + ranks['Rank_Categorical'])/2
ranks = ranks.sort_values('avg', ascending=False)
```
...
```{python}
def style_table(df):
    return (df.style.format({'Test R^2': '{:.3f}','Test RMSE': '{:,.0f}','ZIP Total Importance (share)': '{:.2%}'})
            .set_table_styles([
            ...
            .background_gradient(cmap='Blues', subset=['Test R^2'])
