...
# One split on row indices, reused across models (ensures fair A/B)
idx = np.arange(len(df))
train_idx, test_idx = train_test_split(idx, test_size=0.20, random_state=123)
```
...
```{python}
# Feature/target for numeric case
X_num = df.drop('SalePrice', axis=1)
y_num = df['SalePrice']

X_tr = X_num.iloc[train_idx]
X_te = X_num.iloc[test_idx]
y_tr = y_num.iloc[train_idx]
y_te = y_num.iloc[test_idx]

# Slightly more permissive tree so ZIP can compete for splits if it carries signal
...
# Metrics (version-agnostic formulas)
...
# Reuse the same rows
Xc_tr = Xc.iloc[train_idx]
Xc_te = Xc.iloc[test_idx]
yc_tr = yc.iloc[train_idx]
yc_te = yc.iloc[test_idx]
...
#| fig-height: 5
fig = plt.figure(figsize=(9,5))
plot_tree(tree_cat, feature_names=list(Xc_tr.columns), filled=True, rounded=True, fontsize=8, max_depth=6)
...
# Sort by max share across models for stable order
...
1) **Diverging delta bars** — (Categorical − Numeric) importance share per feature, sorted by absolute change.  
2) **Compact delta table** — Numeric vs Categorical with ▲/▼ and a centered bar to visualize the delta.

```{python}
# Compute deltas
delta_df = comp.copy()
delta_df['Delta'] = delta_df['Categorical'] - delta_df['Numeric']
delta_sorted = delta_df.sort_values('Delta', key=lambda s: s.abs(), ascending=True).reset_index(drop=True)
```
...
ax.barh(y, delta_sorted['Delta'])
ax.axvline(0, linewidth=1)
ax.set_yticks(y, delta_sorted['Feature'])
ax.set_xlabel("Categorical − Numeric (importance share)")
ax.xaxis.set_major_formatter(ticker.PercentFormatter(xmax=1, decimals=0))
ax.set_title("Diverging Delta Bars — Which model gives each feature more share?")
for yi, val in zip(y, delta_sorted['Delta']):
    ax.text(val + (0.01 if val>=0 else -0.01)*max(0.01, abs(delta_sorted['Delta']).max()),
            yi, f"{val*100:.1f}%", va='center',
            ha='left' if val>=0 else 'right', fontsize=9)
plt.tight_layout(); plt.show()
```

```{python}
# Build compact delta table
tab = delta_df[['Feature','Numeric','Categorical','Delta']].copy()
tab = tab.sort_values('Delta', ascending=False).reset_index(drop=True)

def arrow(v):
    return "▲" if v>0 else ("▼" if v<0 else "•")

tab['Direction'] = tab['Delta'].apply(arrow)

def style_delta(df):
    return (df.style
            .format({'Numeric':'{:.2%}','Categorical':'{:.2%}','Delta':'{:.2%}'})
            .set_table_styles([
            ...
            .hide(axis='index')
            .bar(subset=['Delta'], align='mid', vmin=-df['Delta'].abs().max(), vmax=df['Delta'].abs().max()))
style_delta(tab)
```
...
**Note:** Both models use the **same** train/test rows (identical split), ensuring a fair apples‑to‑apples comparison.
...
```{python}
if zip_total > 0:
    msg = f"<p><strong>Finding:</strong> Treating ZIP as a category revealed measurable neighborhood signal — ZIP contributed <strong>{zip_total*100:.2f}%</strong> of total importance. Numeric ZIP hid this signal behind arbitrary thresholds. For interpretability around location, categorical encoding is the right choice.</p>"
else:
    msg = f"<p><strong>Finding:</strong> In this sample and model depth, the tree did not split on ZIP even when encoded categorically (ZIP share <strong>{zip_total*100:.2f}%</strong>). That means neighborhood labels (ZIP) did not add predictive value beyond the existing numeric features. It is still correct to treat ZIP as categorical; this dataset simply shows little incremental neighborhood signal under these constraints.</p>"
HTML(msg)
```
...
**Recommendation:** Model ZIP **as a categorical, non‑ordinal** variable. ZIP codes label neighborhoods; there is no meaningful order among codes. Encoding ZIP as numbers forces artificial thresholds. Encoding as categories (via one‑hot or, better, a model with native categorical support) lets the tree separate neighborhoods directly. The narrative above is conditional on the measured ZIP share, so the text always reflects the data on this page.

## 2) R vs Python when ZIP is categorical
- **R (rpart)** treats string/factor columns as **factors** natively, allowing splits on subsets of factor levels without manual encoding.  
- **Python (sklearn.tree.DecisionTreeRegressor)** requires **numeric input**; categorical features must be encoded (one‑hot or ordinal). This changes the split search space and how importance credit is distributed across dummies.
- Documentation notes that categorical variables are not supported natively and must be converted to numeric before fitting.

## 3) Better categorical handling in Python 
When categories carry signal (like ZIP), consider models with **native categorical support**:
- **CatBoost** (handles categorical features internally with ordered statistics; avoids manual one‑hot).
- **LightGBM** (supports categorical features via integer encoding and searches category splits efficiently).

**Practical guidance:** Keep the scikit‑learn baseline here (for teaching), but call out that production models with location categories should consider **CatBoost** or **LightGBM** to avoid distortions from manual one‑hot encoding.
